<template>
  <input type="file" accept=".dxf" @change="changeFile" placeholder="请选择dxf文件" />
...
const init = () => {
  ...
  console.log(scene);
  // let material = new THREE.LineBasicMaterial({ linewidth: 10, color:  new THREE.Color(0x00ffff)});
  // let points = [new THREE.Vector3(2096,1271,0),new THREE.Vector3(0,0,0)]
...
const changeFile = (event) => {
  ...
  reader.onload = (e) => {
    ...
    console.log(dxf);
    const data = dxf
    const res = new ThreeDxf.Viewer(dxf, document.getElementById('dxfContainer'), 1000, 800);
    console.log(res);
    ...
    for (let i = 0; i < data.entities.length; i++) {
        ...
        let obj = drawEntity(entity, data);
        if (obj) {
          scene.scene.add(obj)
        }
        obj = null;
...
function drawSpline(entity, data) {
  ...
  var points = getBSplinePolyline(entity.controlPoints, entity.degreeOfSplineCurve, entity.knotValues, 100, undefined);
...
function drawLine(entity, data) {
  ...
  for (i = 0; i < entity.vertices.length; i++) {
    ...
    if (entity.vertices[i].bulge) {
      ...
      let bulgePoints = getBulgeCurvePoints(startPoint, endPoint, bulge, undefined);
...
function drawDimension(entity, data) {
  ...
  for (var i = 0; i < block.entities.length; i++) {
    var childEntity = drawEntity(block.entities[i], data);
